...
function renderCard(c, q) {
  ...
  <article class="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 overflow-hidden flex flex-col">
    ...
    <div class="p-4 flex flex-col flex-grow">
      <div class="text-xs text-gray-500" data-course-code="${c.code}">${
    t.courseCode
  } ${code}</div>
...
function renderListItem(c, q) {
  ...
  const t = I18N[STATE.lang];
  return `
  <div class="relative bg-white rounded-lg shadow-md p-4 flex flex-col sm:flex-row gap-4 items-start sm:items-center hover:shadow-lg transition-shadow duration-300">
    <a href="details.html?code=${c.code}" data-details-link data-course-code="${
    c.code
  }" class="block w-full sm:w-32 flex-shrink-0">
      <img src="${imgSrc}" onerror="this.src='images/default.png'" alt="${
    c.name
  }" class="w-full h-32 sm:h-24 object-cover rounded-md zoomable" data-large="${imgSrc}">
    </a>
    ...
    t.courseCode
          ...
          <h3 class="font-semibold text-gray-800 text-lg mt-1">${name}</h3>
          <div class="text-sm text-gray-500 mt-1">${t.startDate}: ${formatDate(
    c.startDate
  )} &nbsp; | &nbsp; ${t.capacity}: ${c.capacity || "-"}</div>
        </div>
        <div class="mt-4 sm:mt-0 sm:ml-4 flex-shrink-0 flex flex-col items-start sm:items-end gap-2">
          <div class="text-teal-600 font-bold text-xl">${formatPrice(
            c.price
          )}</div>
          <div class="flex gap-2">
            <button data-signup-link 
                    ...
                    data-course-start-date="${c.startDate}"
                    data-course-image="${imgSrc}"
                    class="btn-primary px-3 py-1.5 rounded-md transition-colors text-sm">${
                      t.enroll
                    }</button>
            <button data-details-link 
                    data-course-code="${c.code}"
                    class="btn-secondary px-3 py-1.5 rounded-md transition-colors text-sm">${
                      t.details
                    }</button>
          </div>
        </div>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // --- Favorite (heart) button on details page ---
  try {
    ...
    if (favBtn) {
      ...
      const saveFavs = (arr) =>
        localStorage.setItem(COURSE_KEY, JSON.stringify(arr));

      // Use page course code if present in DOM
      const courseCodeEl = document.getElementById("courseCode");
      const courseCode = courseCodeEl
        ? courseCodeEl.textContent.replace(/\s*/g, "").split("：").pop()
        : "unknown_course";
    ...
    console.error("Favorite button init error", err);
  }
});
